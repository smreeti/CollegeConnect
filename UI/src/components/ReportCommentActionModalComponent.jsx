...
import { API_TO_APPROVE_COMMENT_REPORTS, API_TO_APPROVE_POST_REPORTS, API_TO_REJECT_COMMENT_REPORTS, API_TO_REJECT_POST_REPORTS, API_TO_REPORT_POST } from "../../utils/APIRequestUrl";
import fetchData from "../../utils/FetchAPI";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

const ReportCommentActionModalComponent = (props) => {
    const reportModal = useRef(null);

    const [remarks, setRemarks] = useState('');
    const [errors, setErrors] = useState('');
    const { selectedCommentReportId, action } = props;
    ...
    const confirmAction = async () => {
        ...
        if (Object.keys(formErrors).length > 0) {
            ...
            const reportData = {
                commentReportsId: selectedCommentReportId,
                remarks
            };

            if (action == "APPROVE") {
                const data = await fetchData(API_TO_APPROVE_COMMENT_REPORTS, "POST", reportData);
...
export default ReportCommentActionModalComponent;