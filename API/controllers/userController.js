...
const fetchMasterUser = async (collegeId) => {
  const masterUser = await User.findOne({
    userTypeId: {
      $in: await UserType.findOne({ code: "MASTER_ADMIN" }),
    },
    collegeInfoId: collegeId,
  });

  console.log(masterUser, "Master user");
...
const editProfile = async (req, res) => {
  const loggedInUser = req.user;
  const { firstName, lastName, email, mobileNumber, username, bio } = req.body;
